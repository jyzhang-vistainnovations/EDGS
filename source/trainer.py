...
class EDGSTrainer:
    ...
    def load_checkpoints(self, load_cfg):
        ...
        if load_cfg.gs:
            ...
            self.CONSOLE.print(
                f"3DGS loaded from checkpoint for iteration {load_cfg.gs_step}",
                style="info",
            )
    ...
    def load_ply(self, path):
        self.GS.gaussians.load_ply(path)
    ...
    def evaluate(self):
            ...
            },
            {
                ...
                "cameras": [
                    self.scene.getTrainCameras()[
                        idx % len(self.scene.getTrainCameras())
                    ]
        ...
        if self.log_wandb:
            with torch.no_grad():
                ...
                wandb.log(
                    {"time": self.timer.get_elapsed_time()}, step=self.training_step
                )
        torch.cuda.empty_cache()
    ...
    def train_step_gs(self, max_lr=False, no_densify=False):
        ...
        loss = (
            1.0 - self.training_config.lambda_dssim
        ) * L1_loss + self.training_config.lambda_dssim * ssim_loss
        self.timer.pause()
    ...
    def init_with_corr(self, cfg, verbose=False, roma_model=None):
        ...
        if not cfg.add_SfM_init:
            with torch.no_grad():
                ...
                self.gaussians.tmp_radii = torch.zeros(self.gaussians._xyz.shape[0]).to(
                    self.device
                )
                mask = torch.concat(
                    [
                        ...
                        torch.zeros(
                            N_splats_after_init - N_splats_at_init, dtype=torch.bool
                        ),
                    ],
                    axis=0,
                )
